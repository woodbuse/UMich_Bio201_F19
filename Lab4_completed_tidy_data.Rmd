...
scfa_data %>%
  group_by(semester) %>%
  summarize(mean_butyrate = mean(butyrate_mM_per_kg_rounded, na.rm = TRUE))
```
...
```{r}
scfa_data %>%
  filter(!is.na(butyrate_mM_per_kg_rounded)) %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mM_per_kg_rounded))
```
...
scfa_data %>%
 filter(!is.na(butyrate_mM_per_kg_rounded)) %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mM_per_kg_rounded), 
            sd_butyrate = sd(butyrate_mM_per_kg_rounded), 
            n_butyrate = n())
```
...
# write code here
scfa_data %>%
  filter(!is.na(butyrate_mM_per_kg_rounded & acetate_mM_per_kg_rounded & propionate_mM_per_kg_rounded)) %>%
  group_by(study_week) %>%
  summarize(mean_butyrate = mean(butyrate_mM_per_kg_rounded), 
            sd_butyrate = sd(butyrate_mM_per_kg_rounded), 
            n_butyrate = n(),
  mean_acetate = mean(acetate_mM_per_kg_rounded), 
            sd_acetate = sd(acetate_mM_per_kg_rounded), 
            n_acetate = n(),
    mean_propionate = mean(propionate_mM_per_kg_rounded), 
            sd_propionate = sd(propionate_mM_per_kg_rounded), 
            n_propionate = n())
...
It takes week 3 and week 1 mean butyrate data for each individual and shows all the other variables and outputs for those groupings but shows the week1 mean butyrate and week3 mean butyrate in separate columns.
```{r}
scfa_wkly_qc %>%
  spread(study_week, butyrate_mM_per_kg_mean)
```
...
```{r}
scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, butyrate_mM_per_kg_mean) %>% #drop extra columns
...
butyrate_wide <- scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, butyrate_mM_per_kg_mean) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, butyrate_mM_per_kg_mean) %>% #convert to wide format
...
acetate_wide <- scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, acetate_mM_per_kg_mean) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, acetate_mM_per_kg_mean) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
...
propionate_wide <- scfa_wkly_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, propionate_mM_per_kg_mean) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, propionate_mM_per_kg_mean) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
...
```{r}
# write code here
average_fecal_butyrate_difference <- scfa_wkly_qc %>%
  select(participant_id, supplement_consumed, study_week, butyrate_mM_per_kg_mean) %>%
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, butyrate_mM_per_kg_mean) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
  mutate(delta_butyrate = week3 - week1) 
average_fecal_butyrate_difference
```

Repeat this process for each SCFA. How would you plot your results? 
```{r}
# copy+paste and edit code chunk for acetate
average_fecal_acetate_difference <- scfa_wkly_qc %>%
  select(participant_id, supplement_consumed, study_week, acetate_mM_per_kg_mean) %>%
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, acetate_mM_per_kg_mean) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
  mutate(delta_acetate = week3 - week1)
average_fecal_acetate_difference
```

```{r}
# copy+paste and edit code chunk for propionate
average_fecal_propionate_difference <- scfa_wkly_qc %>%
  select(participant_id, supplement_consumed, study_week, propionate_mM_per_kg_mean) %>%
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, propionate_mM_per_kg_mean) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
  mutate(delta_propionate = week3 - week1)#calculate difference 
average_fecal_propionate_difference
```

```{r}
# copy+paste and edit code chunks for plots
average_fecal_butyrate_difference_plot <- ggplot(data = butyrate_wide, 
...
average_fecal_butyrate_difference_plot

average_fecal_acetate_difference_plot <- ggplot(data = acetate_wide, 
       mapping = aes(x = supplement_consumed, 
                     y = delta_acetate)) +
...
average_fecal_acetate_difference_plot

average_fecal_propionate_difference_plot <- ggplot(data = propionate_wide, 
       mapping = aes(x = supplement_consumed, 
                     y = delta_propionate)) +
...
average_fecal_propionate_difference_plot
```
...
```{r}
# write code here
total_scfa_stats <- inner_join(acetate_wide, butyrate_wide,
                    by = c("participant_id", "semester", "supplement_consumed"))%>%
  inner_join(propionate_wide,
                    by = c("participant_id", "semester", "supplement_consumed"))%>%
  mutate( scfa_wk1 = ace_wk1 + but_wk1 + pro_wk1) %>%
  mutate( scfa_wk3 = ace_wk3 + but_wk3 + pro_wk3) %>%
  mutate( delta_scfa = scfa_wk3 - scfa_wk1) 
...
```{r}
# write code here
delta_scfa_plot <- ggplot(data = total_scfa_stats, 
       mapping = aes(x = semester, 
                     y = delta_scfa, supplement_consumed,
                     color = supplement_consumed)) +
    geom_point() + 
    labs(title = "Delta SCFA Concentrations of Individuals",
         x = "Semester",
         y = "Delta SCFA (mmol/kg)")
...
total_scfa_stats %>%
  ...
  rename(acetate = delta_acetate,
         ...
         total = delta_scfa) %>% 
  gather(key = SCFA, value = wkly_dqiff,
...
total_scfa_stats %>%
  ...
  gather(key = SCFA_wk, value = SCFA_concentration,
         ace_wk1, ace_wk3, but_wk1, but_wk3, pro_wk1, pro_wk3, scfa_wk1, scfa_wk3)
```
...
# acetate long
total_scfa_stats
ace_long <- total_scfa_stats %>%
  #drop diff columns & extra SCFAs
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("pro_"), -starts_with("scfa")) %>% 
...
but_long <- total_scfa_stats %>%
  #drop diff columns & extra SCFAs
  select(-starts_with("delta_"), -starts_with("ace_"), -starts_with("pro_"), -starts_with("scfa")) %>% 
...
total_scfa_stats
pro_long <- total_scfa_stats %>%
  #drop diff columns & extra SCFAs
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("ace_"), -starts_with("scfa")) %>% 
...
wkly_means <- scfa_long %>%
  gather(key = week, value = wkly_mean_scfa,
      study_week ,concentration_mmol_kg, scfa)
...
pH_qc <- pH_data %>%
  ...
  group_by(participant_id, ph, study_week) %>%
  summarise(mean(ph)) #calculate weekly average
dim(pH_qc)
pH_qc
```
...
BSS_qc <- BSS_data %>%
  ...
  group_by(study_week, bristol_numeric, participant_id) %>%
  summarise(mean(bristol_numeric)) #calculate weekly average 
dim(BSS_qc)
...
```{r}
pH_BSS_wkly <- inner_join(pH_qc, BSS_qc,
                    by = c(participant_id, study_week)) %>%
  mutate(round(pH_BSS_wkly, 2)) #round avgs to reflect sig figs
```
