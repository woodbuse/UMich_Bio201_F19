...
nrow(scfa_indv) # it tells us how many rows of data frame there are
ncol(scfa_indv) # it tells us how many coloums of the data frame there are
dim(scfa_indv) # shows us how many rows and coloums (in that order) of the data frame
colnames(scfa_indv) # as the name suggests, it displays the names of the coloums in the order they appear (left to right)
rownames(scfa_indv) # as the name suggests, it displays the names of the rows in the order they appear (up to down) but only to the 1000th row, then another function is needed to see the rest of the row names
glimpse(scfa_indv) # gives us a glimpse of the data by showing us how many observations  there are followed by the amount of variables. It then proceeds to list the variables with the first couple observations for each variable. It also displays whether the function is a character vector, dlb, etc.
str(scfa_indv) # similar to glimpse, it shows the amount of observations, variables, lists the variables and a few data points (observations) for each. The format is slightly different as it just says if the observations are character or numeric rather than character or double. Also it is not a rectangle presented when the code runs or everything, it looks cleaner. Additionally, at the top it shows the "classes" this data fits in.
```
...
mutate(scfa_indv) #create new columns by using information from other columns
group_by(scfa_indv) %>% summarize(scfa_indv) #create summary statistics of grouped data
arrange(scfa_indv) #sort results
count(scfa_indv) #count discrete values
```
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(notes != "frozen>24hrs") %>%
  filter(sample_wt_g > 0.10) %>% 
  filter(sample_wt_g < 1.0) %>% 
  select(participant_id, sex, age, `race/ethnicity`, ht_in, wt_lbs, sample_number, sample_id, sample_wt_g, butyrate_mM)
...
scfa_indv <- read_excel("raw_data/SCFA_data.xlsx",
 ...
 scfa_indv_qc <- scfa_indv %>%
  rename_all(tolower) %>%
  rename(sample_wt_g = tube_wt) %>%
  rename(acetate_mM = ace, 
         butyrate_mM = but,
         propionate_mM = pro) %>%
  mutate(wt_kg = wt_lbs / 2.205) %>%
  mutate(ht_m = ht_in / 35.274) %>% 
  mutate(ht_m_rounded = round(ht_m, 0)) %>%
  mutate(wt_kg_rounded = round(wt_kg, 0)) %>%
  filter(sample_wt_g > 0.10) %>% 
  filter(sample_wt_g < 1.0) %>% 
  filter(notes != "frozen>24hrs") %>%
  mutate(sample_wt_kg = sample_wt_g / 1000) %>%
  mutate(acetate_mM_per_kg = (acetate_mM*.002) / sample_wt_kg) %>%
  mutate(butyrate_mM_per_kg = (butyrate_mM*.002) / sample_wt_kg) %>%
  mutate(propionate_mM_per_kg = (propionate_mM*.002) / sample_wt_kg) %>%
  mutate(total_scfa_mM_per_kg = propionate_mM_per_kg + butyrate_mM_per_kg + acetate_mM_per_kg) %>%
  mutate(acetate_mM_per_kg_rounded = round(acetate_mM_per_kg, 2)) %>%
  mutate(butyrate_mM_per_kg_rounded = round(butyrate_mM_per_kg, 2)) %>%
  mutate(propionate_mM_per_kg_rounded = round(propionate_mM_per_kg, 2)) %>%
  mutate(total_scfa_mM_per_kg_rounded = round(total_scfa_mM_per_kg, 2)) %>%
  select(participant_id, sex, age, 'race/ethnicity', sample_number, sample_id, study_week, semester, supplement_consumed, use_data, quantity_compliant, frequency, notes, ht_m_rounded, wt_kg_rounded, sample_wt_kg, acetate_mM_per_kg_rounded, butyrate_mM_per_kg_rounded, propionate_mM_per_kg_rounded, total_scfa_mM_per_kg_rounded)
...
```{r}
scfa_wkly_qc <- read_excel("raw_data/SCFA_wkly.xlsx", sheet = "SCFA_wkly", col_names = TRUE, trim_ws = TRUE, na = c("", "NA", "-----")) %>% 
  rename_all(tolower) %>%
  rename(acetate_mM_median = acetate_median, 
         acetate_mM_mean = acetate_mean,
         butyrate_mM_median = butyrate_median, 
         butyrate_mM_mean = butyrate_mean,
         propionate_mM_median = propionate_median, 
         propionate_mM_mean = propionate_mean,) %>%
  filter(study_week == "week1" | study_week == "week3") %>%
  filter(quantity_compliant == "yes") %>%
  mutate(mean_total_scfa = (acetate_mM_mean + butyrate_mM_mean + propionate_mM_mean)) %>%
  mutate(median_total_scfa = (acetate_mM_median + butyrate_mM_median + propionate_mM_median))
```


scfa_wkly_qc <- read_delim(file = "raw_data/SCFA_wkly.txt", 
delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"), col_types = list())
